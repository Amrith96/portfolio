...
function ProjectModal({ project, onClose }: { project: Project; onClose: () => void }) {
  return (
    <>
      ...
      />
      <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, zIndex: 51, width: '100%', maxWidth: 560, background: '#0d0d0d', borderLeft: '1px solid rgba(255,255,255,0.08)', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '32px 32px 24px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ width: 48, height: 48, borderRadius: 12, marginBottom: 16, background: 'rgba(232,0,11,0.1)', border: '1px solid rgba(232,0,11,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <p style={{ fontSize: 10, letterSpacing: '0.2em', color: '#E8000B', textTransform: 'uppercase', marginBottom: 4 }}>{project.number} — {project.type}</p>
        ...
        <div style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 32 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {[{ label: 'Status', value: project.status }, { label: 'Year', value: project.year }, { label: 'Role', value: project.role }].map(m => (
              <div key={m.label} style={{ background: '#151515', borderRadius: 10, padding: 16, border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {project.results.map(r => (
                  <div key={r.value + r.label} style={{ background: '#151515', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10, padding: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {project.stack.map(tech => (
                <span key={tech} style={{ border: '1px solid rgba(255,255,255,0.1)', borderRadius: 9999, padding: '4px 12px', fontSize: 11, color: '#888', letterSpacing: '0.05em' }}>{tech}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '24px 32px', borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 'auto' }}>
          <a
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#E8000B', color: '#fff', borderRadius: 4, padding: '16px 24px', textDecoration: 'none', transition: 'background 0.2s' }}
...
function ProjectRow({ project, index, onClick }: { project: Project; index: number; onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', transition: 'border-color 0.3s' }}
      onMouseEnter={e => {
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
        <span className="bebas proj-num" style={{ fontSize: 48, color: 'rgba(255,255,255,0.1)', lineHeight: 1, width: 56, transition: 'color 0.3s' }}>
          {project.number}
        </span>
        <div>
          <p className="bebas proj-title" style={{ fontSize: 22, color: '#F5F5F0', transition: 'color 0.3s', lineHeight: 1.1 }}>
            {project.shortTitle}
          </p>
          <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
            {project.tags.map(tag => (
              <span key={tag} style={{ fontSize: 10, letterSpacing: '0.1em', color: '#555', border: '1px solid rgba(255,255,255,0.08)', padding: '2px 8px', borderRadius: 2, textTransform: 'uppercase' }}>
                {tag}
              </span>
            ))}
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexShrink: 0 }}>
        <span style={{ fontSize: 10, letterSpacing: '0.15em', color: '#555', textTransform: 'uppercase' }}>{project.type}</span>
        <span className="proj-arrow" style={{ fontSize: 20, color: 'rgba(255,255,255,0.2)', transition: 'color 0.3s, transform 0.3s' }}>↗</span>
      </div>
    </motion.div>
  )
}
  ...
  return (
    <section id="projects" style={{ background: '#0a0a0a', padding: '96px 32px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        {/* Header */}
        <div ref={headerRef} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 64 }}>
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={headerInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.7 }}
          >
            <p style={{ color: '#E8000B', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 12 }}>/ Selected Projects</p>
          ...
          <motion.p
            ...
            style={{ fontSize: 11, letterSpacing: '0.15em', color: '#555', textTransform: 'uppercase', cursor: 'pointer' }}
          >
            8 Projects →
          </motion.p>
        </div>
